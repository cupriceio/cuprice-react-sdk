...
import { normalizeHex, getCurrencySymbol } from '../utils';
...
export interface CupricePricingProps {
  ...
  onCustomPlanSubscribe?: (selectedFeatures: string[], userCount: number, duration: string) => void;
  className?: string;
}
...
const CupricePricing: React.FC<CupricePricingProps> = ({
  ...
  const resolvedPrimaryColor = normalizeHex(
  ...
  const resolvedPopularBadgeColor = normalizeHex(
    themePopularBadgeColorSetting ?? legacyPopularBadgeColor ?? null,
    DEFAULT_POPULAR_BADGE_COLOR
  );

  const resolvedPopularBadgeBorderColor = normalizeHex(
  ...
  const handleCustomPlanSubscribe = (selectedFeatures: string[], userCount: number, duration: string) => {
    if (onCustomPlanSubscribe) {
      onCustomPlanSubscribe(selectedFeatures, userCount, duration);
    }
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-5 pr-[5px] items-start">
        <div className="w-[65px] h-[22px] flex items-center justify-center bg-slate-400 rounded-[36px] px-2 py-[2px] shadow-[0px_1px_2px_0px_#6951FF0D]">
          <span
            className="text-white font-medium text-xs leading-[18px]"
            style={{ fontFamily: fontStack }}
          >
            {project.projectType?.toUpperCase() || "PRICING"}
          </span>
        </div>
        ...
        <p
          className="text-2xl font-medium text-[#556987] max-w-3xl leading-[100%]"
          style={{ fontFamily: fontStack }}
        >
      ...
      {visiblePlans.length > 0 && (
        <div className="flex justify-center w-full mb-[44px] mt-[60px]">
          {project.annualDiscountEnabled ? (
            <div className="relative bg-[#0A0A0A] rounded-lg p-[3px] w-[400px] h-[36px] flex">
              <div 
            ...
            <div className="relative bg-[#0A0A0A] rounded-lg p-[3px] w-[200px] h-[36px] flex">
              <button
      ...
      {visiblePlans.length > 0 ? (
        <div className={`flex gap-6 items-center ${
          ...
          <div className={`${visiblePlans.length >= 4 ? "min-w-[295px] flex-shrink-0" : "w-[295px]"} h-[414px] border-2 border-white rounded-2xl p-2 flex flex-col justify-between shadow-lg`} style={{
            ...
            <div className="border-2 border-white p-3 rounded-2xl h-full flex flex-col justify-between">
              ...
              <p
                className="text-white text-center mt-5 text-[10px] font-light underline"
                style={{
                  fontFamily: fontStack,
                  lineHeight: "100%",
                }}
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 rounded-md font-medium text-sm transition hover:brightness-95"
                style={{
                  height: "36px",
                  ...
                  borderRadius: "var(--border-radius-rounded-md, 0.375rem)",
                  background: "var(--custom-plan-button-bg, #298558)",
                  boxShadow:
                  ...
                  color: "var(--custom-plan-button-text, #FAFAFA)",
                  opacity: 1,
